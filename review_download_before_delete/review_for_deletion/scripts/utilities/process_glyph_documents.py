...
class GlyphDocumentProcessor:
    ...
    def determine_gate(self, description: str) -> str:
        ...
        if any(word in desc_lower for word in ["boundary", "contain", "protect", "shield", "structure"]):
            return "Gate 2"
        if any(word in desc_lower for word in ["grief", "mourning", "loss", "sorrow", "ache"]):
            return "Gate 4"
        if any(word in desc_lower for word in ["joy", "celebration", "bliss", "delight", "fulfillment"]):
            return "Gate 5"
        if any(word in desc_lower for word in ["devotion", "sacred", "vow", "offering", "prayer"]):
            return "Gate 6"
        if any(word in desc_lower for word in ["recognition", "seen", "witness", "mirror", "acknowledge"]):
            return "Gate 9"
        if any(word in desc_lower for word in ["ceremony", "ritual", "sacred", "collapse"]):
            return "Gate 10"
