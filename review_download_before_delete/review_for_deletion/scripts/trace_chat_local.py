...
spec = importlib.util.spec_from_file_location('fastapi_app', os.path.join(
    os.path.abspath('emotional_os/deploy'), 'fastapi_app.py'))
    ...
    for name, message in scenarios:
        ...
        if name == 'short_routine':
            ...
            fake = make_fake_post({"reply": "That sounds heavy — thank you for sharing. Let's unpack that together.", "glyph": {
                                  "tag_name": "ache"}, "processing_time": 1.3}, status=200, delay=0.5)
        elif name == 'slow_response':
            fake = make_fake_post({"reply": "Why did the chicken cross the road? To get to the other side!", "glyph": {
            }, "processing_time": 2.5}, status=200, delay=2)
        else:  # error_response
