...
DB_PATH = os.path.join(os.path.dirname(__file__), '..',
                       'emotional_os', 'glyphs', 'glyphs.db')
DB_PATH = os.path.normpath(DB_PATH)


# Path to optional curated CSV (display_name,response_template)
CURATED_CSV = os.path.join(os.path.dirname(
    __file__), 'output', 'curated_top20_glyphs.csv')
...
def migrate():
    ...
    if curated_pairs:
        print(
            f"Applying {len(curated_pairs)} curated mappings from CSV: {CURATED_CSV}")
    ...
    if applied:
        ...
        for r in rows:
            print(
                f" - {r[0]} -> {r[1]} | template: { (r[2][:80] + '...') if r[2] and len(r[2])>80 else r[2] }")
