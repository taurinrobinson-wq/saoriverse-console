...
def main():
    if not DB.exists():
        print(f"glyphs.db not found at {DB}; run import_glyphs_json_to_sqlite.py first")
        return
    ...
    try:
        ...
        if "base_emotion" not in cols:
            cur.execute("ALTER TABLE glyph_lexicon ADD COLUMN base_emotion TEXT")
            conn.commit()
        ...
        for rowid, name, desc in rows:
            ...
            if emo:
                cur.execute("UPDATE glyph_lexicon SET base_emotion = ? WHERE rowid = ?", (emo, rowid))
                updated += 1
        conn.commit()
