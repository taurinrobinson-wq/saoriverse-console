...
def main():
    parser = argparse.ArgumentParser(
        description="Save & load a test conversation via ConversationManager")
    parser.add_argument('--user-id', required=True,
                        help='User id to use when saving the conversation (string or UUID)')
    parser.add_argument('--conv-id', default=None,
                        help='Optional conversation id (defaults to random uuid)')
    args = parser.parse_args()

    user_id = args.user_id
    conv_id = args.conv_id or str(uuid.uuid4())

    print(f"Using SUPABASE_URL={os.environ.get('SUPABASE_URL')} (masked) and service role key present? {'YES' if os.environ.get('SUPABASE_SERVICE_ROLE_KEY') else 'NO'}")
    ...
    ok, msg = mgr.save_conversation(
        conv_id, title, messages, processing_mode='local')
    print("save:", ok, msg)
