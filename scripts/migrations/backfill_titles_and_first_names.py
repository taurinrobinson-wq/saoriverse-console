...
SUPABASE_KEY = os.getenv('SUPABASE_SERVICE_ROLE_KEY') or os.getenv(
    'SUPABASE_SERVICE_ROLE') or os.getenv('SUPABASE_KEY')
...
def _get_conversations_missing_title(limit: int = 200) -> List[Dict[str, Any]]:
    ...
    if resp.status_code != 200:
        logger.error('Failed to query conversations (HTTP %s): %s',
...
def _get_messages_missing_first_name(limit: int = 500) -> List[Dict[str, Any]]:
    ...
    resp = requests.get(url, headers=HEADERS, params=params, timeout=15)
    if resp.status_code != 200:
        logger.error('Failed to query messages (HTTP %s): %s',
                     resp.status_code, resp.text)
        return []
    return resp.json()
...
def _try_fetch_profile_first_name(user_id: str) -> Optional[str]:
    ...
    params = {'select': 'first_name,username',
...
def _patch_conversation_title(conversation_id: str, new_title: str, apply: bool = False) -> bool:
    ...
    if not apply:
        logger.info("DRY-RUN: would PATCH conversation %s -> title=%s",
    ...
    if r.status_code in (200, 204):
        ...
        return True
    logger.error("Failed to patch conversation %s (HTTP %s): %s",
                 conversation_id, r.status_code, r.text)
    return False
...
def _patch_message_first_name(message_id: Any, new_first_name: str, apply: bool = False) -> bool:
    ...
    if not apply:
        logger.info("DRY-RUN: would PATCH message %s -> first_name=%s",
                    message_id, new_first_name)
        return True
    r = requests.patch(url, headers=HEADERS, params=params,
                       json=payload, timeout=10)
    if r.status_code in (200, 204):
        ...
        return True
    logger.error("Failed to patch message %s (HTTP %s): %s",
                 message_id, r.status_code, r.text)
    return False


def main(argv: List[str]):
    parser = argparse.ArgumentParser(
        description='Backfill conversation titles and message first_names')
    ...
    msg_updates = 0
    for m in msgs:
        mid = m.get('id') or m.get('message_id') or m.get(
