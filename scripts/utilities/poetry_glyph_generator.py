...
class PoetryGlyphGenerator:
    ...
    def extract_glyph_patterns(self, min_combined_freq: int = 300) -> List[Dict]:
        """Extract emotional patterns that should become glyphs.

        Args:
            min_combined_freq: minimum combined frequency for a pair to be considered
        """
        patterns = []
        ...
        dimensions = sorted(self.signals.keys(), key=lambda d: self.signals[d].get(
            "frequency", 0), reverse=True)
