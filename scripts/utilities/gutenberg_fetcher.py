...
class GutenbergPoetryFetcher:
    ...
    def download_poetry_collection(self, name, book_id):
        ...
        self.downloaded_poems.append(
            {"name": name, "path": str(file_path), "book_id": book_id, "size": len(text), "words": len(text.split())}
        )
...
def process_all_poetry():
    ...
    for poetry_file in fetcher.get_downloaded_files():
        ...
        try:
            stats = processor.process_file(poetry_file, user_id="gutenberg_bulk", chunk_size=500)
            ...
            logger.info(
                "  ✓ Processed: {} new entries, {} signals".format(
                    stats.get("total_new_lexicon_entries", 0), stats.get("total_signals", 0)
                )
            )
    ...
    if hasattr(processor.extractor, "get_dimension_report"):
        ...
        logger.info(
            "  Pre-discovered dimensions: {}".format(dimension_report["breakdown"]["pre_discovered_dimensions"])
        )
