...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Bulk process text files through Saoriverse learning system")
    parser.add_argument("--file", type=str, help="Path to single text file to process")
    parser.add_argument("--dir", type=str, help="Path to directory containing text files")
    parser.add_argument(
        "--pattern", type=str, default="*.txt", help="File pattern for directory processing (default: *.txt)"
    )
    parser.add_argument("--chunk-size", type=int, default=500, help="Word count per chunk (default: 500)")
    parser.add_argument("--user-id", type=str, default="bulk_processor", help="User ID for learning attribution")
    parser.add_argument("--output", type=str, default="bulk_processing_results.json", help="Output file for results")
