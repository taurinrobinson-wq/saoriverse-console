...
try:
    ...
    # Fallbacks (conservative): disallow remote AI and provide an error raiser
...
class EvolvingGlyphIntegrator:
    ...
    def __init__(
        ...
        if (
            ...
            and SUPABASE_INTEGRATOR_AVAILABLE
            and SupabaseIntegrator is not None
        ):
            # If user provided explicit credentials but remote AI is disabled, raise
            ...
            try:
                self.supabase_integrator = SupabaseIntegrator(
                    function_url=supabase_function_url, supabase_anon_key=supabase_anon_key
                )
    ...
    def process_conversation_with_evolution(
        self, message: str, mode: str = "quick", conversation_context: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            if self.supabase_integrator:
                saori_response = self.supabase_integrator.process_message(
                    message=message, mode=mode, conversation_context=conversation_context or {}
                )
            ...
            should_evolve = (
                self.enable_auto_evolution
                and self.glyph_generator
                and (
                    self.conversation_count % self.evolution_frequency == 0
                    or self._has_strong_emotional_content(message)
                )
            )
    ...
    def force_evolution_check(self, conversation_text: str, context: Optional[Dict] = None) -> List[Dict]:
        ...
        try:
            new_glyphs = self.glyph_generator.process_conversation_for_glyphs(
                conversation_text=conversation_text, context=context or {}
            )
...
def conversation_demo_with_evolution():
    ...
    for i, conv in enumerate(test_conversations, 1):
        ...
        result = integrator.process_conversation_with_evolution(
            message=conv["message"], conversation_context=conv["context"]
        )
