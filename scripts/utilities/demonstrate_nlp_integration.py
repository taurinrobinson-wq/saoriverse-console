...
def demonstrate_enhancement():
    ...
    for i, text in enumerate(test_inputs, 1):
        ...
        print(f"   Dominant Emotion: {enhanced_analysis['dominant_emotion']} (confidence: {enhanced_analysis['confidence']:.2f})")
        print(f"   NRC Emotions: {enhanced_analysis['nrc_emotions']}")
        print(f"   TextBlob Sentiment: polarity={enhanced_analysis['textblob_sentiment']['polarity']:.2f}, subjectivity={enhanced_analysis['textblob_sentiment']['subjectivity']:.2f}")
        ...
        response_preview = result['voltage_response'][:100] + "..." if len(result['voltage_response']) > 100 else result['voltage_response']
