...
class LexiconAwareResponseGenerator:
    ...
    def generate_response(
        ...
        learned_contexts = self._find_learned_contexts(message_keywords, user_lexicon)
    ...
    def _build_context_understanding(
        ...
        if len(learned_contexts) >= 3 or any(
            len(ctx.get("associated_emotions", [])) > 2 for _, ctx in learned_contexts
        ):
    ...
    def _build_question(
        ...
        if primary_emotion:
            return (
                f"When you feel this {primary_emotion} about '{keyword}'—what part of it asks for something from you?"
            )
