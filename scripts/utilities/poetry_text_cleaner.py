...
class PoetryTextCleaner:
    ...
    def clean_text(self, text: str, verbose: bool = False) -> str:
        ...
        if verbose:
            logger.info(f"Cleaning: {original_length} → {len(text)} chars ({100*(1-len(text)/original_length):.1f}% removed)")
...
class PoetryTextValidator:
    ...
    def validate_file(self, filepath: Path) -> Dict:
        ...
        results['checks']['fragmentation'] = {
            'pass': frag_markers < 50,
            'marker_count': frag_markers
        }
