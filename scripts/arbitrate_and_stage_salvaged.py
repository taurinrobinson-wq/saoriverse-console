...
def decide_gate_for_group(group):
    ...
    gates = [(m.get("gate") or "").strip() for m in members if (m.get("gate") or "").strip()]
    gates_set = set(gates)
    if len(gates_set) == 1:
    ...
    for m in members:
        ...
        combined_text.append(
            (m.get("description") or "") + " " + (m.get("title") or "") + " " + (m.get("glyph_name") or "")
        )
...
def update_salvaged_db(salvaged_idx):
    ...
    for gid, rec in salvaged_idx.items():
        cur.execute(
            "UPDATE salvaged_glyphs SET gate=?, notes=?, confidence=? WHERE group_id=?",
...
def create_stage_table_and_insert():
    ...
    cur.execute(
        """CREATE TABLE glyph_lexicon_salvaged (
        group_id INTEGER,
        source_rowids TEXT,
        glyph_name TEXT,
        title TEXT,
    ...
    cur.execute(
        "SELECT group_id, source_rowids, glyph_name, title, description, gate, emotional_tags, activation_signals, confidence, notes FROM salvaged_glyphs"
    )
    rows = cur.fetchall()
    for r in rows:
        cur.execute(
            f"INSERT INTO {STAGED_TABLE} (group_id, source_rowids, glyph_name, title, description, gate, emotional_tags, activation_signals, confidence, notes) VALUES (?,?,?,?,?,?,?,?,?,?)",
            r,
        )
    conn.commit()
    # count rows
...
def main():
    ...
    for g in flagged:
        ...
        resolutions.append(
            {"group_id": gid, "resolved_gate": resolved_gate, "confidence": conf, "rationale": rationale}
        )
    ...
    with open(FLAGGED_CSV, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["group_id", "resolved_gate", "confidence", "rationale"])
        writer.writeheader()
        for r in resolutions:
            writer.writerow(r)

    # update salvaged jsonl and db
    update_salvaged_jsonl(
        salvaged_idx,
    )
