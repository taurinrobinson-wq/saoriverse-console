...
def main():
    ...
    for user_text, ai_text in exchanges:
        res = learner.learn_from_exchange(user_id, user_text, ai_text, emotional_signals=None, glyphs=None)
        results.append(res)
    ...
    report = {
        ...
        "lexicon_results_summary": (
            lex_results.get("summary", lex_results) if isinstance(lex_results, dict) else lex_results
        ),
    }
    ...
    print("shared signals in lexicon:", hybrid_stats.get("signals_in_shared_lexicon"))
    print("user signals learned:", user_stats.get("signals_learned"))
    print("\nTop user signals (if any):")
    overrides_file = Path(learner.user_overrides_dir) / f"{user_id}_lexicon.json"
    if overrides_file.exists():
        try:
            u = load_json(overrides_file)
            signals = u.get("signals", {})
            for sig, data in signals.items():
                print(f" - {sig}: {len(data.get('keywords', []))} keywords, freq={data.get('frequency')}")
        except Exception as e:
