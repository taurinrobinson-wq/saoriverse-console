...
cm_path = os.path.join(os.getcwd(), 'emotional_os',
                       'deploy', 'modules', 'conversation_manager.py')
...
def main():
    ...
    for i, text in enumerate([
        ...
        messages.append(msg)
        ok, res_msg = mgr.save_conversation(
            conv_id, f"E2E test conv {conv_id[:8]}", messages, processing_mode='local')
