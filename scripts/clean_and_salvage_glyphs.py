...
def strip_export_artifacts(text):
    ...
    patterns = [r"CORE PRINCIPLES:.*", r"TITLE:\s*", r"📅.*", r"JSON Export.*", r"\bVELΩNIX\b", r"— JSON Export.*"]
    for p in patterns:
    ...
    for line in s.splitlines():
        if re.search(
            r"EXPORT|EXPORTED|MARKDOWN EXPORT|JSON EXPORT|CONVERSATION ARCHIVE|TITLE:", line, flags=re.IGNORECASE
        ):
            continue
...
def infer_gate_and_emotions(glyph_name, description, fallback):
    ...
    gm = re.search(r"gate[:\s]+([A-Za-z0-9_\-]+)", description or "", flags=re.IGNORECASE)
    if gm:
...
def process_csv(inpath, outpath, fallback):
    ...
    with open(inpath, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            glyph_name = canonicalize_name(row.get("glyph_name") or row.get("display_name") or "")
            ...
            if not gate_orig or True:
                g, emos, conf, mats = infer_gate_and_emotions(glyph_name, desc_clean, fallback)
                matches = mats
    ...
    for name, items in grouped.items():
        ...
        items_sorted = sorted(
            items, key=lambda x: (x.get("confidence", 0), len(x.get("description") or "")), reverse=True
        )
        canonical = items_sorted[0]
        cleaned.append(canonical)
        # flag near-duplicates
        for other in items_sorted[1:]:
            sim = similar(canonical.get("description", "")[:200], other.get("description", "")[:200])
            if sim > 0.86:
                # likely duplicate; skip but note
                canonical["notes"] = (canonical.get("notes") or "") + f'; dedup:{other["source_rowid"]}'
            else:
...
def main():
    ...
    out = sys.argv[2] if len(sys.argv) > 2 else os.path.join(OUT_DIR, os.path.basename(inp).replace(".csv", ".jsonl"))
    ...
    print("Processed:", summary["in_count"], "->", summary["out_count"], "cleaned records")
