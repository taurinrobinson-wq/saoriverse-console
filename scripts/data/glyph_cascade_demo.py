"""Glyph Cascade Demo with Chained Transitions
----------------------------------------------
Maps glyphs → emotions → harmonic functions → chord progressions,
favoring smoother transitions (root absorption) using `shared_notes`.
...
def find_best_transition(function, pivot, prev_notes=None):
    """Return the (to, shared_notes) with strongest overlap; weight by prev_notes if provided."""
    candidates = []
    for entry in pivot:
