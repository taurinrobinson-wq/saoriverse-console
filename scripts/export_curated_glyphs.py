...
cur.execute("SELECT rowid, glyph_name, display_name, response_template, gate FROM glyph_lexicon WHERE display_name IS NOT NULL AND display_name != '' AND response_template IS NOT NULL AND response_template != '' ORDER BY display_name COLLATE NOCASE")
rows = cur.fetchall()

with open(OUT_PATH, 'w', newline='', encoding='utf-8') as fh:
    writer = csv.writer(fh)
    writer.writerow(['rowid', 'glyph_name', 'display_name',
                    'response_template', 'gate'])
    for r in rows:
