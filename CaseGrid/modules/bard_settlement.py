...
module_path = scripts_dir / "bard_pdf_extract.py"
spec = importlib.util.spec_from_file_location("bard_pdf_extract", module_path)
if spec is None or spec.loader is None:
    raise ModuleNotFoundError(f"Could not load helper module: {module_path}")
_bard_pdf_extract = importlib.util.module_from_spec(spec)
spec.loader.exec_module(_bard_pdf_extract)

build_new_pdf_name = _bard_pdf_extract.build_new_pdf_name
extract_fields_from_pdf = _bard_pdf_extract.extract_fields_from_pdf
process_pdf_file = _bard_pdf_extract.process_pdf_file
unique_path = _bard_pdf_extract.unique_path

INPUT_EXT = ".pdf"
_PREFIX = "bard_settlement_"
...
def run() -> None:
    ...
    st.header("Bard Settlement Processor")
    st.caption(
    ...
    with col3:
        if st.button("Clear Files", key=_k("clear")):
            ...
            st.session_state.pop(_k("duplicates_removed"), None)
            st.rerun()
    ...
    with col2:
        if st.button("Process Batch", type="primary", disabled=not uploaded_files, key=_k("process")):
            with st.spinner("Processing uploaded files..."):
                ...
                st.session_state[_k("process_timing")] = (elapsed_seconds, processed_count, total_input_bytes)
                st.session_state[_k("duplicates_removed")] = duplicates_removed
    ...
    duplicates_removed = st.session_state.get(_k("duplicates_removed"), 0)
