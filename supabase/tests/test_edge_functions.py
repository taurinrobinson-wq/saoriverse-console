...
SUPABASE_SERVICE_ROLE_KEY = os.environ.get("SUPABASE_SERVICE_ROLE_KEY") or os.environ.get("PROJECT_SERVICE_ROLE_KEY")

HEADERS_ANON = {"Content-Type": "application/json", "Authorization": f"Bearer {SUPABASE_ANON_KEY}"}
...
if missing:
    pytest.skip(f"Skipping Supabase tests, missing env vars: {', '.join(missing)}", allow_module_level=True)
...
def test_create_and_authenticate_user():
    ...
    r = requests.post(SUPABASE_AUTH_URL, json=payload, headers=HEADERS_ANON, timeout=15)
    assert r.status_code == 200, r.text
    resp = r.json()
    assert resp.get("success") is True and resp.get("user") and resp["user"].get("id")
    user_id = resp["user"]["id"]

    # Authenticate
    r2 = requests.post(
        SUPABASE_AUTH_URL,
        json={"action": "authenticate", "username": username, "password": "P@ssw0rdTest"},
...
def test_saori_fixed_anonymous():
    ...
    r = requests.post(url, json={"message": "hello test", "mode": "quick"}, headers=headers, timeout=20)
    assert r.status_code == 200, r.text
    j = r.json()
...
def test_authenticated_saori_with_custom_token():
    ...
    r = requests.post(
        url, json={"message": "I feel anxious", "mode": "quick", "user_id": user_id}, headers=headers, timeout=20
    )
    assert r.status_code == 200, r.text
    j = r.json()
    assert "reply" in j
