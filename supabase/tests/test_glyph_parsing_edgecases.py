from supabase.tests.glyph_utils import is_valid_glyph_shape, normalize_glyphs
...
def test_name_and_description_truncation_and_depth_clamp():
    long_name = "n" * 200
    long_desc = "d" * 1000
    raw = [{"name": long_name, "description": long_desc, "depth": 10}, {"name": "a", "depth": -5}]
...
def test_optional_fields_copied_and_truncated():
    raw = [{"name": "x", "response_layer": "r" * 200, "glyph_type": "t"}, {"name": "y", "symbolic_pairing": 123}]
    out = normalize_glyphs(raw)
