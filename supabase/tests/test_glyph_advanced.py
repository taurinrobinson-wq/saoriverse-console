...
from supabase.tests.test_openai_error_paths import (
    FakeOpenAIError,
    call_openai_with_handling,
)
...
def test_extract_json_from_markdown_wrapper():
    payload = {"glyphs": [{"name": "chest_tightening", "description": "A tight chest."}]}
...
def test_ambiguous_name_formats_are_kept_and_truncated():
    ...
    raw = [{"name": long_name, "description": "d"}, {"name": "weird/name,with:chars", "description": "x"}]
    out = normalize_glyphs(raw)
