...
class GlyphFactorialEngine:
                ...
                for row in reader:
                    if row:
                        glyph = {
                            ...
                            'gate': row.get('gate', ''),
                            'activation_signals': row.get('activation_signals', '').split(',') if row.get('activation_signals') else []
                        }
                        glyphs.append(glyph)
            ...
            for j, glyph2 in enumerate(self.primary_glyphs):
                ...
                combination = GlyphCombination(
                    ...
                    activation_signals=self.combine_activation_signals(
                        glyph1["activation_signals"],
                        glyph2["activation_signals"]
                    ),
    ...
    def prune_combinations(self, keep_top_percent: float = 0.10, 
                          min_score_threshold: float = 0.5) -> Tuple[int, int]:
        ...
        self.combinations = [c for c in self.combinations 
                            if c.parent_ids[0] != c.parent_ids[1]]
    ...
    def sync_to_json(self, approved_combinations: Optional[List[GlyphCombination]] = None, 
