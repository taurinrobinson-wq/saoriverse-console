...
def combined_factorial_and_pruning_pipeline():
    ...
    for i in range(sample_size):
        for j in range(sample_size):
            if i <= j:  # Upper triangle only
                ...
                voltage_pair = factorial_engine.combine_voltage_pairs(
                    glyph1['voltage_pair'], glyph2['voltage_pair']
                )
                name = factorial_engine.generate_combination_name(
                    glyph1['glyph_name'], glyph2['glyph_name']
                )
                description = factorial_engine.generate_combination_description(
                    glyph1['description'], glyph2['description'],
                    glyph1['glyph_name'], glyph2['glyph_name']
                )
                signals = factorial_engine.combine_activation_signals(
                    ','.join(glyph1.get('activation_signals', [])),
                    ','.join(glyph2.get('activation_signals', []))
                )
                gate = factorial_engine.estimate_gate(
                    (glyph1.get("gate", "Gate 5"), glyph2.get("gate", "Gate 5")),
                    signals
                )
    ...
    pruning_engine = AdvancedPruningEngine(
        glyph_lexicon_path=temp_json_path,
    ...
    if pruned_candidates:
        archive_path = pruning_engine.archive_pruned_glyphs(
            pruned_candidates, 
            reason="factorial_expansion_pruning"
        )
