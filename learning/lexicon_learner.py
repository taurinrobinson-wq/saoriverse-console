...
class LexiconLearner:
    ...
    def _map_word_to_signal(self, word: str, context: Dict) -> str:
        ...
        if any(pattern in word_lower for pattern in ['ache', 'long', 'yearn', 'crave']):
            return signal_mapping['longing_words']
        if any(pattern in word_lower for pattern in ['joy', 'happy', 'delight', 'bliss']):
            return signal_mapping['joy_words']
        if any(pattern in word_lower for pattern in ['protect', 'guard', 'safe', 'bound']):
            return signal_mapping['protection_words']
        if any(pattern in word_lower for pattern in ['see', 'hear', 'witness', 'notice']):
            return signal_mapping['recognition_words']
        if any(pattern in word_lower for pattern in ['sacred', 'devot', 'honor', 'reverent']):
            return signal_mapping['devotion_words']
        if any(pattern in word_lower for pattern in ['clear', 'understand', 'insight', 'realize']):
