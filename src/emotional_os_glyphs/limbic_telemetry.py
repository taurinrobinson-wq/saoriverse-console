...
def fetch_recent(n: int = 20) -> List[Dict[str, Any]]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT timestamp, user_id, input_text, emotion, enrichment_applied, ab_group, latency_ms, glyphs_generated, safety_flag FROM limbic_events ORDER BY id DESC LIMIT ?",
        (n,),
    )
