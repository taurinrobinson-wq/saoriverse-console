...
class HybridLearnerWithUserOverrides:
    ...
    def __init__(
        ...
        if self.anonymization_enabled:
            self.anonymizer = AnonymizationProtocol(allow_medical=allow_medical_details, allow_names=allow_names)
    ...
    def learn_from_exchange(
        ...
        try:
            ...
            self._learn_to_user_lexicon(user_overrides, user_input, ai_response, emotional_signals)
            self._save_user_overrides(user_id, user_overrides)
            result["learned_to_user"] = True

            # 3. Check if quality enough for shared lexicon
            is_quality, reason = self._is_quality_exchange(user_input, ai_response, emotional_signals)
    ...
    def _log_exchange(
        ...
        try:
            ...
            if self.anonymization_enabled and self.anonymizer:
                try:
                    entry = {"text": user_input, "metadata": {"timestamp": datetime.now().isoformat()}}
                    anonymized_entry, anonymization_map = self.anonymizer.anonymize_entry(entry, user_id)
        ...
        if emotional_signals:
            for signal_dict in emotional_signals:
                ...
                if signal and keyword:
                    ...
                    signal_context = {
                        "keyword": keyword,
                        "associated_signals": [s.get("signal") for s in emotional_signals if s.get("signal") != signal],
                        "gates": list(set([s.get("gate") for s in emotional_signals if s.get("gate")])),
        ...
        if user_id:
            ...
            return {
                ...
                "learning_log_entries": (
                    sum(1 for _ in open(self.learning_log_path)) if Path(self.learning_log_path).exists() else 0
                ),
                "scope": "shared",
            }
