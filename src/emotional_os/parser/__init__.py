...
try:
    import emotional_os_parser as _sibling_module
    
    # Register all submodules from the sibling in sys.modules under emotional_os namespace
    for key, module in list(sys.modules.items()):
        if key.startswith('emotional_os_parser'):
            # Map sibling.X to emotional_os.parser.X
            new_key = key.replace('emotional_os_parser', 'emotional_os.parser')
    ...
    import warnings
    warnings.warn(f"Failed to import emotional_os_parser: {e}")
