...
try:
    import emotional_os_safety as _sibling_module
    
    # Register all submodules from the sibling in sys.modules under emotional_os namespace
    for key, module in list(sys.modules.items()):
        if key.startswith('emotional_os_safety'):
            # Map sibling.X to emotional_os.safety.X
            new_key = key.replace('emotional_os_safety', 'emotional_os.safety')
    ...
    import warnings
    warnings.warn(f"Failed to import emotional_os_safety: {e}")
