...
try:
    import emotional_os_privacy as _sibling_module
    
    # Register all submodules from the sibling in sys.modules under emotional_os namespace
    for key, module in list(sys.modules.items()):
        if key.startswith('emotional_os_privacy'):
            # Map sibling.X to emotional_os.privacy.X
            new_key = key.replace('emotional_os_privacy', 'emotional_os.privacy')
    ...
    import warnings
    warnings.warn(f"Failed to import emotional_os_privacy: {e}")
