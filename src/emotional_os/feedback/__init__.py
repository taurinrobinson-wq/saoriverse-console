...
try:
    import emotional_os_feedback as _sibling_module
    
    # Register all submodules from the sibling in sys.modules under emotional_os namespace
    for key, module in list(sys.modules.items()):
        if key.startswith('emotional_os_feedback'):
            # Map sibling.X to emotional_os.feedback.X
            new_key = key.replace('emotional_os_feedback', 'emotional_os.feedback')
    ...
    import warnings
    warnings.warn(f"Failed to import emotional_os_feedback: {e}")
