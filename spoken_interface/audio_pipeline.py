...
class AudioProcessor:
    ...
    def load_audio(audio_bytes: bytes, sr: int = 16000) -> Tuple[np.ndarray, int]:
        ...
        if not LIBROSA_AVAILABLE:
            raise ImportError("librosa required for audio loading. Install: pip install librosa")
...
class AudioPipeline:
    ...
    def process_user_audio(self, audio_bytes: bytes) -> dict:
        ...
        try:
            ...
            result["vad_ratio"] = float(np.sum(vad_mask) / len(vad_mask))  # % of audio with voice
