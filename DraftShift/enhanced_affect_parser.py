...
This module provides advanced emotion detection for tone analysis in legal correspondence.
...
class EnhancedAffectAnalysis:
    ...
    dominance: float  # 0 (low control) to 1 (high control)
...
class EnhancedAffectParser:
        ...
        if use_spacy:
            try:
                ...
                try:
                    ...
                    # Model not found; fall back to blank pipeline
                    # (Downloads should happen during deployment setup, not runtime)
                    try:
                        self.spacy_model = spacy.blank("en")
                        logger.warning("⚠️ SpaCy model not available; using blank 'en' pipeline. To use full model, run: python -m spacy download en_core_web_sm")
                    except Exception as e3:
                        logger.warning(f"SpaCy load failed: {e3}")
            except ImportError:
    ...
    def _create_fallback_nrc(self):
        """Create minimal fallback NRC lexicon for tone analysis."""
        fallback = {
            # Joy/Positive words
            ...
            "good": {"joy": 0.5, "positive": 1.0},
            "great": {"joy": 0.5, "positive": 1.0},
            "wonderful": {"joy": 1.0, "positive": 1.0},
            "appreciate": {"joy": 0.5, "positive": 1.0, "trust": 1.0},
            "thank": {"positive": 1.0, "trust": 1.0},
            "glad": {"joy": 1.0, "positive": 1.0},
            "pleased": {"joy": 1.0, "positive": 1.0},
            
            # Sadness/Negative words
            ...
            "upset": {"sadness": 1.0, "negative": 1.0},
            "disappointed": {"sadness": 1.0, "negative": 1.0},
            ...
            "pursuant": {"trust": 0.5},
            "accordingly": {"positive": 0.5},
            "formal": {"positive": 0.5},
            "professional": {"positive": 1.0, "trust": 1.0},
            "sincerely": {"positive": 1.0, "trust": 1.0},
            "respectfully": {"positive": 1.0, "trust": 1.0},
        }
        self.nrc_lexicon = fallback
        logger.info("Fallback NRC lexicon created with legal-specific terms")
