"""
Opposition — Opposition Class for California Civil Pleadings

Represents an Opposition to a Motion.
Inherits formatting + citation rules from BaseDocument.
Exposes a clean build() method for assembling oppositions.
"""
...
class Opposition(BaseDocument):
    ...
    Structure:
        ...
        4. Document title (opposition title)
        5. Arguments (structured headings + paragraphs)
        6. Signature block
        7. Proof of Service (separate page)

    Oppositions are structurally identical to Replies:
        - No Notice section
        - Standard caption + argument structure
    """

    def __init__(self, config_path: str, citation_path: str):
        """
        Initialize Opposition with formatting and citation configs.
    ...
    def build(self, data: dict) -> None:
        """
        Builds a complete opposition using structured data.
        ...
        Expected data structure:
        {
            ...
            "title": "OPPOSITION TO MOTION FOR NEW TRIAL",
            "arguments": [
                {"level": 1, "text": "INTRODUCTION"},
                {"paragraph": "Defendant's motion should be denied..."},
                {"level": 1, "text": "ARGUMENT"},
                {"level": 2, "text": "I. No Legal Error"},
                {"paragraph": "The evidence clearly supports..."},
                ...
            ],
            "pos": {
                "server_name": "...",
    ...
    def _build_document_title(self, title: str) -> None:
        ...
        Args:
            title: Opposition title (e.g., "OPPOSITION TO MOTION FOR NEW TRIAL")
        """
        p = self.doc.add_paragraph()
        run = p.add_run(title)
        run.bold = True
