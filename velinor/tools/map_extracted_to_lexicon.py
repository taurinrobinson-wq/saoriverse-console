...
EXTRACTED = [
    {"name": "feeling_unmoored", "phrase": "I feel unmoored and need something to hold onto"},
    {"name": "seeking_stability", "phrase": "I feel unmoored and need something to hold onto"},
...
lines = []
lines.append(
    "extracted_name,source_phrase,matched,top_match,match_score,match_type,suggested_synonyms,suggested_triggers,new_glyph"
)
...
for ex in EXTRACTED:
    ...
    match_type = "exact" if tover > 0 or tsubstr else ("fuzzy" if tsim > 0.65 else "none")
    ...
    if ex_name in ["feeling_unmoored", "seeking_stability"]:
        suggested_synonyms = ["adrift", "ungrounded", "anchored", "stability", "secure"]
    elif ex_name in ["anger", "dismissal"]:
        suggested_synonyms = ["angry", "irate", "dismissed", "rejected", "undervalued"]
    ...
    suggested_triggers = []
    if ex_name == "feeling_unmoored":
        suggested_triggers = ["unmoored", "need something to hold onto", "adrift"]
    if ex_name == "seeking_stability":
        suggested_triggers = ["need something to hold onto", "seek stability", "looking for solid ground"]
    if ex_name == "anger":
        suggested_triggers = ["furious", "angry", "they dismissed me", "I was dismissed"]
    if ex_name == "dismissal":
        suggested_triggers = ["dismissed", "they dismissed me", "felt disregarded"]
    if ex_name == "longing":
        suggested_triggers = ["I miss her", "I miss them", "yearning", "I miss"]
    if ex_name == "heartache":
        suggested_triggers = ["it aches", "heartache", "hurts so much"]
    if ex_name == "quiet_joy":
        suggested_triggers = ["quiet joy", "watching them sleep", "serene happiness"]
    if ex_name == "overwhelm":
        suggested_triggers = ["overwhelmed", "everything I'm carrying", "burdened by"]
    ...
    if ex_name in ["longing", "heartache"]:
        ...
        if "long" in top_name.lower() or "ache" in top_name.lower():
            ...
            new_glyph = "no"
            match_type = "token"
    # write CSV line (escape double-quotes in fields)
    suggested_synonyms_csv = "|".join(suggested_synonyms).replace('"', '""')
    suggested_triggers_csv = "|".join(suggested_triggers).replace('"', '""')
    ex_phrase_csv = ex_phrase.replace('"', '""')
    top_name_csv = top_name.replace('"', '""')
    line = f'{ex_name},"{ex_phrase_csv}",{matched},"{top_name_csv}",{top_score:.3f},{match_type},"{suggested_synonyms_csv}","{suggested_triggers_csv}",{new_glyph}'
    lines.append(line)
