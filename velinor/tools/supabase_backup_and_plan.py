...
def fetch_all_rows(supabase_url: str, supabase_key: str, table: str) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        r = requests.get(base, headers={**headers, "Range": f"items={start}-{end}"}, params={"select": "*"})
        if r.status_code not in (200, 206):
            raise RuntimeError(f"Failed to fetch rows: {r.status_code} {r.text}")
        page = r.json()
        if not page:
            break
        rows.extend(page)
...
def build_upsert_plan(backup: List[Dict[str, Any]], cleaned: List[Dict[str, Any]], key: str) -> Dict[str, Any]:
    ...
    """
    by_key_backup = {str(r.get(key)): r for r in backup if key in r and r.get(key) is not None}
    by_key_cleaned = {str(r.get(key)): r for r in cleaned if key in r and r.get(key) is not None}
    ...
    deletions = [r for k, r in by_key_backup.items() if k not in by_key_cleaned]
...
def rows_equal(a: Dict[str, Any], b: Dict[str, Any]) -> bool:
    ...
    for k in a.keys():
        if a[k] == b[k]:
            continue
        try:
            if json.dumps(a[k], sort_keys=True, ensure_ascii=False) == json.dumps(
                b[k], sort_keys=True, ensure_ascii=False
            ):
                continue
        except Exception:
            pass
...
def parse_args():
    ...
    p.add_argument("--supabase-url", help="Supabase URL (overrides SUPABASE_URL env)")
    p.add_argument("--supabase-key", help="Supabase Service Role key (overrides SUPABASE_KEY env)")
    p.add_argument("--table", default="glyphs", help="Table name to backup")
    p.add_argument("--out-dir", default=DEFAULT_OUT_DIR)
    p.add_argument("--cleaned", default=os.path.join(DEFAULT_OUT_DIR, "cleaned_glyphs.json"))
    p.add_argument("--key", default="slug", help="Canonical key to match rows for upsert (default: slug)")
    p.add_argument(
        "--no-backup", action="store_true", help="Skip remote fetch; only build plan from local cleaned file"
    )
    return p.parse_args()
...
def main():
    ...
    if args.no_backup:
        ...
        if not supabase_url or not supabase_key:
            print(
                "SUPABASE_URL and SUPABASE_KEY are required to fetch a live backup.\n"
                "If you prefer to only stage a plan from local data, re-run with --no-backup."
            )
            sys.exit(1)
        print(f"Fetching table '{args.table}' from Supabase at {supabase_url}...")
        ...
        print(f"Wrote backup: {backup_json} and {backup_csv} ({len(backup_rows)} rows)")
    ...
    print(summary)
    print(
        "Next: review these files, verify the plan, then run dev_tools/supabase_upsert_runner.py --apply with credentials to perform the upsert in safe batches."
    )
