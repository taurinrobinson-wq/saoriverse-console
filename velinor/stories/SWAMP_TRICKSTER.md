...
Brief: A duplicitous swamp informant that functions as a TONE mirror and gatekeeper to the thieves'
hideout (Drossel). The encounter rewards players for emotional alignment, observation, and narrative
presence while punishing blind trust or authoritarian alignment.
...
Integration instructions: 1. If using `StoryBuilder` in `story_definitions.py`, import and parse
`swamp_trickster_scene.json` and iterate passages:
...
2. Implement tone application logic in your engine if not present: when a player selects a choice,
add each numeric delta to hidden TONE stats and persist to session state. 3. Implement `tone_checks`
evaluation immediately after choice resolution; if a check matches, redirect to the gate outcome
instead of the default `to` target. 4. Wire REMNANTS influence lines in your NPC manager so the
trickster's influence on Kaelen/Drossel/Veynar changes downstream behaviors.
