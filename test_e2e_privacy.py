...
# Set up path
sys.path.insert(0, '/Users/taurinrobinson/saoriverse-console')
            ...
            if log_path.exists():
                ...
                if logged_entries:
                    ...
                    checks = {
                        ...
                        "✅ HAS signals": "signals" in logged_entries[0] and isinstance(logged_entries[0]["signals"], list),
                    ...
                    if "signals" in lexicon:
                        for signal_name, signal_data in list(lexicon["signals"].items())[:1]:  # Show first signal
                            ...
                            if "example_contexts" in signal_data:
                                ...
                                print(f"      First example: {signal_data['examples'][0][:60] if signal_data['examples'] else 'N/A'}...")
            else:
...
def test_learner_only_privacy(tmpdir):
    ...
    print(f"  ✅ HAS metadata: timestamp={bool(entry.get('timestamp'))}, user_id_hash={bool(entry.get('user_id_hash'))}")
