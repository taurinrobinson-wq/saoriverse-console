...
def main():
    expanded_path = Path(
        'learning/user_overrides/openstax_bulk_lexicon_expanded.json')
    glyphs_path = Path('data/generated_glyphs_from_openstax.json')
    out_path = Path(
        'learning/user_overrides/openstax_bulk_lexicon_expanded_triggers.json')
    ...
    for sig, data in expanded.get('signals', {}).items():
        keywords = list(dict.fromkeys([k.strip().lower()
                        for k in data.get('keywords', []) if k]))
        signals[sig] = {
