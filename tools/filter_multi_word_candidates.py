...
def main():
    ...
    with IN_CSV.open("r", encoding="utf-8") as f:
        ...
        for idx, row in enumerate(reader):
            ...
            if not phrase or " " not in phrase:
                continue
            doc = nlp(phrase)
            first_alpha = next((tok for tok in doc if not tok.is_space and not tok.is_punct), None)
            if first_alpha is None or first_alpha.is_stop:
                continue
            tokens_alpha = [tok for tok in doc if (tok.is_alpha or tok.like_num) and not tok.is_punct]
            if len(tokens_alpha) < 2:
                continue
            alpha_tokens = [tok for tok in tokens_alpha if re.search(r"[A-Za-z]", tok.text)]
            ...
            if any(t in stop_blacklist for t in lower_tokens):
                continue
            noun_like = sum(1 for tok in tokens_alpha if tok.pos_ in ("NOUN", "PROPN", "ADJ"))
    ...
    with OUT_TSV.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow(["phrase", "lemmas", "pos", "example", "source_urls", "noun_score"])
        for r in filtered:
            writer.writerow(
                [
                    r.get("phrase", ""),
