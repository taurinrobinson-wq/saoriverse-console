...
def main():
    ...
    with st.sidebar:
        st.header("Options")
        view_mode = st.radio("View Mode:", ["Document View", "Raw Content", "Metadata", "Export"])
        ...
        selected_file = st.selectbox("Select a DOCX file:", docx_files, format_func=lambda x: x.name)
    ...
    if view_mode == "Document View":
        ...
        st.download_button(
            label="⬇️ Download as TXT", data=text_content, file_name=f"{selected_file.stem}.txt", mime="text/plain"
        )
