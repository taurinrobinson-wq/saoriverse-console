...
This directory contains scripts to create a full backup of the `glyphs` table from Supabase and a
conservative, reviewable upsert plan built from the local cleaned export.
...
1. Review the plan files generated by `supabase_backup_and_plan.py` (look for
`dev_tools/supabase_upsert_plan_<ts>.json`, `*_inserts.csv`, `*_updates.csv`). Verify the counts and
...
3. When you're satisfied, provide credentials and apply the upsert from a safe environment
(Codespace or CI) where the service-role key is available as an environment variable. Example
(Codespace or local):
...
4. Monitor the output. The runner will stop on the first failing batch. If you see errors, fix the
issues (often malformed JSON fields or constraint conflicts) and re-run the runner against the
failing subset.
...
If you want, I can run the backup now — provide SUPABASE_URL and SUPABASE_KEY in the Codespace
secrets or tell me to skip and let you run it locally. I will not run the upsert until you
explicitly give the go-ahead.
