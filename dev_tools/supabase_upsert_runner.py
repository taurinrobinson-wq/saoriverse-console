...
def parse_args():
    ...
    p.add_argument(
        '--plan', help='Path to supabase_upsert_plan_*.json', default=None)
    p.add_argument('--inserts-csv',
                   help='CSV of records to insert', default=None)
    p.add_argument('--updates-csv',
                   help='CSV of records to update (new values)', default=None)
    p.add_argument('--supabase-url', help='Supabase URL (overrides ENV)')
    p.add_argument('--supabase-key',
                   help='Supabase service role key (overrides ENV)')
    p.add_argument('--table', default='glyphs')
    p.add_argument('--on-conflict', default='slug',
                   help='on_conflict key (default: slug)')
    p.add_argument('--batch-size', type=int, default=200,
                   help='Records per HTTP batch')
    p.add_argument('--apply', action='store_true',
                   help='Actually perform network POSTs (default: dry-run)')
    return p.parse_args()
...
def apply_batches(url: str, key: str, table: str, on_conflict: str, rows: List[Dict[str, Any]], batch_size: int, do_apply: bool):
    total = len(rows)
    print(
        f"Preparing to {'apply' if do_apply else 'dry-run'} {total} rows to {table} in batches of {batch_size}")
    i = 0
    success_count = 0
    errors = []
    while i < total:
...
def main():
    ...
    result = apply_batches(supabase_url, supabase_key, args.table,
                           args.on_conflict, rows, args.batch_size, args.apply)
