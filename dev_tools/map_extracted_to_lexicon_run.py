...
EXTRACTED = [
    {"name": "feeling_unmoored",
        "description": "A sense of instability and lack of direction."},
    {"name": "seeking_stability",
        "description": "The desire for something solid to hold onto."},
    {"name": "anger", "description": "Furious feelings about being dismissed at work."},
    {"name": "dismissal",
        "description": "Feeling disregarded and undervalued in a professional setting."},
    {"name": "longing", "description": "A deep sense of yearning for someone who is absent."},
    {"name": "heartache",
        "description": "Emotional pain stemming from missing someone dearly."},
    {"name": "quiet_joy", "description": "A serene happiness found in peaceful moments."},
    {"name": "overwhelm",
        "description": "Feeling burdened by excessive responsibilities and emotions."}
]
...
for row in lex:
    ...
    entries.append({'id': gid, 'glyph_name': glyph_name, 'norm_name': norm_name,
                   'snake': snake, 'description': desc, 'row': row})
...
with OUT_CSV.open('w', encoding='utf-8', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=['extracted_name', 'extracted_description', 'match_type', 'match_score',
                            'matched_id', 'matched_glyph_name', 'suggested_synonyms', 'suggested_triggers', 'new_glyph_needed'])
    writer.writeheader()
    for r in rows_out:
        writer.writerow(r)
