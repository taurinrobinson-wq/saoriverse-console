...
from parser.enhanced_signal_parser import (
    enhanced_parse_input,
...
def demo_conversation():
    ...
    for i, user_input in enumerate(inputs):
        ...
        conversation_context = {"depth": i, "history": conversation_history}

        # Parse with enhanced system
        result = enhanced_parse_input(
            user_input, "parser/signal_lexicon.json", conversation_context=conversation_context
        )

        # Add to history
        conversation_history.append(
            {"input": user_input, "response": result["voltage_response"], "glyphs": result["glyphs"]}
        )
