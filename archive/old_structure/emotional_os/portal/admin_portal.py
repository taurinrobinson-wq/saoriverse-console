...
# Page configuration
st.set_page_config(
    page_title="FirstPerson Admin Portal", page_icon="⚙️", layout="wide", initial_sidebar_state="expanded"
)
...
class AdminDashboard:
    ...
    def render_system_config(self):
        ...
        with col1:
            ...
            default_mode = st.selectbox(
                "Default Processing Mode", ["hybrid", "local"], index=0, help="Default AI processing mode for new users"
            )
            ...
            enable_glyph_creation = st.checkbox(
                "Enable Glyph Creation", value=True, help="Allow automatic emotional glyph creation"
            )
        ...
        with col2:
            ...
            enable_demo_mode = st.checkbox("Enable Demo Mode", value=True, help="Allow users to access demo mode")
        ...
        with col1:
            password_min_length = st.slider("Minimum Password Length", min_value=6, max_value=20, value=6)

            max_login_attempts = st.slider("Max Login Attempts", min_value=3, max_value=10, value=5)

        with col2:
            enable_2fa = st.checkbox(
                "Enable Two-Factor Authentication", value=False, help="Require 2FA for user accounts"
            )

            admin_notifications = st.checkbox(
                "Admin Notifications", value=True, help="Send notifications for system events"
            )
        ...
        if st.button("💾 Save Configuration", type="primary"):
            config = {
                ...
                "session_settings": {"timeout_hours": session_timeout, "enable_demo_mode": enable_demo_mode},
                "security_settings": {
    ...
    def render_api_management(self):
        ...
        for service in api_services:
            ...
            with col1:
                status_emoji = (
                    "🟢" if service["status"] == "operational" else "🟡" if service["status"] == "degraded" else "🔴"
                )
        ...
        with tab1:
            ...
            if openai_enabled:
                openai_model = st.selectbox(
                    "Default Model", ["gpt-4", "gpt-3.5-turbo", "gpt-4-turbo"]
                )  # noqa: F841  # used for UI state
        ...
        with tab3:
            ...
            if pdf_enabled:
                pdf_template_path = st.text_input(
                    "PDF Template Path", value="/templates/"
                )  # noqa: F841  # used for UI state
