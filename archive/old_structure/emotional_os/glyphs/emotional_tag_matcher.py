...
class EmotionalTagMatcher:
    ...
    def select_optimal_persona(self, emotional_tags: List[str], conversation_context: Dict = None) -> str:
        ...
        if any("ache" in tag.lower() or "longing" in tag.lower() for tag in emotional_tags):
            return "oracle"  # Poetic wisdom for longing states

        if any("joy" in tag.lower() or "delight" in tag.lower() for tag in emotional_tags):
            return "friend"  # Celebratory companion for joy

        if any("confusion" in tag.lower() for tag in emotional_tags):
            return "mentor"  # Clarifying guidance for confusion

        return "companion"  # Default gentle presence

    def generate_contextual_response(
        self, emotional_tags: List[str], persona: str, conversation_context: Dict = None
    ) -> Dict:
    ...
    def _get_tone_profile(self, emotional_tags: List[str], persona: str) -> str:
        ...
        if any("joy" in tag.lower() for tag in emotional_tags):
    ...
    def _generate_response_text(
        ...
        if "Devotional Ache" in emotional_tags:
        ...
        if "Spiral Joy" in emotional_tags:
