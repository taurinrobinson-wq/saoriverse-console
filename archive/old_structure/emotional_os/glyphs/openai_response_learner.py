...
class OpenAIResponseLearner:
    ...
    def analyze_openai_response(self, user_message: str, ai_response: str) -> Dict:
        ...
        analysis["learning_opportunities"] = self.identify_learning_patterns(user_message, ai_response, analysis)
    ...
    def analyze_response_structure(self, response: str) -> Dict:
        ...
        structure = {
            ...
            "starts_with_acknowledgment": any(
                ack in sentences[0].lower() for ack in ["i hear", "i sense", "that sounds"]
            ),
    ...
    def store_learning_data(self, analysis: Dict):
        ...
        try:
            ...
            confidence = sum(opp["confidence"] for opp in analysis["learning_opportunities"]) / max(
                1, len(analysis["learning_opportunities"])
            )
    ...
    def process_conversation_learning(self, user_message: str, ai_response: str) -> Dict:
        ...
        learning_summary = {
            ...
            "confidence_score": sum(opp["confidence"] for opp in analysis["learning_opportunities"])
            / max(1, len(analysis["learning_opportunities"])),
            "next_local_candidates": local_suggestions,
        }

        self.logger.info(
            f"Learning complete: {learning_summary['patterns_learned']} patterns, {learning_summary['local_suggestions_available']} local suggestions"
        )
