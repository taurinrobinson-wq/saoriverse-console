...
The following files were detected by GitHub's secret scanner during the recent commits and were
removed from repository history (the `deploy/CoPilot_chunks` directory was scrubbed and history was
force-updated):
...
If you need copies of these files for analysis (for example, to find out where the tokens were
used), extract them from your local reflog/backup before you run the history-scrub steps; after the
scrub they are intentionally removed from the reflog as well.
