...
def test_select_best_glyph_and_returns_glyphs_selected():
    ...
    g1 = make_glyph('Still Insight',
                    'Quiet revelation and noticing', gate='Gate 5')
    ...
    signals = [{'keyword': 'joy', 'signal': 'λ',
                'voltage': 'high', 'tone': 'joy'}]

    result = signal_parser.select_best_glyph_and_response(
        [g1, g2, g3], signals, input_text='I feel joy')
    ...
    assert all(
        'score' in g for g in glyphs_selected), "Each selected glyph should include a 'score'"
    assert all(
        'display_name' in g for g in glyphs_selected), "Each selected glyph should include a 'display_name'"
