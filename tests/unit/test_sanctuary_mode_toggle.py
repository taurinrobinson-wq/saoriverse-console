...
def test_parse_input_uses_sanctuary_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr(sp, 'generate_contextual_response',
                        lambda *a, **k: ('BASE_RESPONSE', {}))
    # Replace ensure_sanctuary_response so we can detect it's invoked
    monkeypatch.setattr(sp, 'ensure_sanctuary_response', lambda input_text,
                        base_response, tone=None: 'SANCTUARY_WRAPPED')

    # Force sanctuary mode on
    monkeypatch.setattr(sp, 'SANCTUARY_MODE', True)

    result = sp.parse_input('I feel tired and small',
                            'velonix_lexicon.json', db_path=':memory:')
