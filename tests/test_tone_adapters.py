...
def test_generate_initiatory_default():
    ...
    assert ("tell me" in r.lower()) or (
        "what about" in r.lower()) or "spark" in r or "opening" in r


def test_generate_archetypal_default():
    r = generate_archetypal_response()
    # Accept inquisitive/holding phrasing.
    assert ("what about" in r.lower()) or ("tell me" in r.lower()
                                           ) or "hold" in r or "honor" in r or "important" in r
