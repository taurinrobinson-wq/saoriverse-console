...
def test_whats_up_treated_as_conversational():
    ...
    assert isinstance(vr, str) and vr.strip(
    ), f"Expected non-empty conversational response, got: {vr}"
...
def test_reciprocal_rotation_and_tone():
    ...
    for seed in range(6):
        _r.seed(seed)
        res = parse_input(
            "what's up", 'emotional_os/parser/signal_lexicon.json')
        responses.add(res.get('voltage_response'))
    # We expect at least 2 distinct variants over several seeds
    assert len(
        responses) >= 2, f"Expected rotation across variants, got: {responses}"


def test_formal_tone_response():
    import random as _r
    _r.seed(0)
    res = parse_input("how are you doing",
                      'emotional_os/parser/signal_lexicon.json')
    assert res.get('response_source') == 'conversational'
    # Formal replies should contain 'How are' or similar formal phrasing
    assert 'how are' in res.get('voltage_response').lower(
    ) or 'how are you' in res.get('voltage_response').lower()
