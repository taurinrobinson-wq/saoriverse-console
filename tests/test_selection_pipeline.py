...
def test_glyph_pruning_filters():
    ...
    glyphs = [
        ...
        make_glyph('📜 Markdown Export — README v1',
                   '### header\ncontent here'),
        make_glyph('Still Recognition',
                   "Being seen without reaction. A gaze that doesn't grasp, just receives.")
    ]

    signals = []
    best_glyph, (response, feedback), source, _selected = signal_parser.select_best_glyph_and_response(
        glyphs, signals, input_text='test')
...
def test_selection_scores_with_signals():
    ...
    best_glyph, (response, feedback), source, _selected = signal_parser.select_best_glyph_and_response(
        glyphs, signals, input_text='I feel anxious')
