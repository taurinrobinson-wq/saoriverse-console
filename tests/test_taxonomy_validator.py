...
TAX_PATH = os.path.join(os.getcwd(), "local_inference", "emotional_taxonomy.json")
...
def test_canonical_taxonomy_schema():
    ...
    if not os.path.exists(TAX_PATH):
        pytest.skip(f"No canonical taxonomy found at {TAX_PATH}; skipping validator")

    tax = load_taxonomy(TAX_PATH)

    # Root structure
    assert_msg(isinstance(tax, dict), "taxonomy must be a JSON object at the root")
    assert_msg(
        "emotional_tags" in tax and isinstance(tax["emotional_tags"], list),
        "root must contain 'emotional_tags' as an array of tag objects",
    )
    assert_msg(
        "escalation_tiers" in tax and isinstance(tax["escalation_tiers"], dict),
    ...
    assert_msg(
        escalation_keys.issubset(allowed_tiers),
    ...
    for idx, tag_obj in enumerate(tax["emotional_tags"]):
        assert_msg(isinstance(tag_obj, dict), f"each emotional tag must be an object (index {idx})")

        # Required field: canonical_tag (string)
        ct = tag_obj.get("canonical_tag")
        assert_msg(isinstance(ct, str) and ct.strip(), f"tag at index {idx} missing or invalid 'canonical_tag'")
        canonical_tags.append(ct)

        # tier must be present and valid
        tier = tag_obj.get("tier")
        assert_msg(isinstance(tier, str) and tier in allowed_tiers, f"tag '{ct}' has invalid 'tier' value: {tier}")
        # ensure tier exists in escalation_tiers
        assert_msg(
            tier in tax["escalation_tiers"],
        ...
        if syns is not None:
            assert_msg(isinstance(syns, list), f"'synonyms' for tag '{ct}' must be a list if present")
            for s in syns:
                assert_msg(isinstance(s, str), f"each synonym for '{ct}' must be a string")

        # editorial_flags if present must be list of strings
        eflags = tag_obj.get("editorial_flags", [])
        if eflags is not None:
            assert_msg(isinstance(eflags, list), f"'editorial_flags' for '{ct}' must be a list if present")
            for ef in eflags:
                assert_msg(isinstance(ef, str), f"editorial_flag '{ef}' for '{ct}' must be a string")
        ...
        if isinstance(eflags, list) and "cluster_sensitive" in eflags:
            assert_msg(
                tier == "tier_2", f"tag '{ct}' marked 'cluster_sensitive' must be assigned to 'tier_2' (found '{tier}')"
            )

    # All canonical_tag values must be unique
    duplicates = [t for t in canonical_tags if canonical_tags.count(t) > 1]
    assert_msg(len(duplicates) == 0, f"duplicate canonical_tag values found: {set(duplicates)}")
    ...
    for tag_obj in tax["emotional_tags"]:
        for s in tag_obj.get("synonyms", []) or []:
            all_syns.append(s)
            assert_msg(s not in canonical_tags, f"synonym '{s}' duplicates an existing canonical_tag")
