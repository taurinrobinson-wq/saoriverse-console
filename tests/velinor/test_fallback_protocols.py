...
class TestToneAnalyzer:
    ...
    def test_consistent_tone_detection(self):
        ...
        is_ambiguous, reason, confidence = self.analyzer.detect_ambiguity("I'm doing okay, having a decent day")
    ...
    def test_voltage_keywords_detection(self):
        ...
        is_ambiguous, reason, confidence = self.analyzer.detect_ambiguity("I'm broken inside, struggling with pain")
    ...
    def test_trigger_misfire_sarcasm(self):
        ...
        is_misfire, reason = self.analyzer.detect_misfire(
            "stay", "Yeah sure, 'stay' with me because that's worked so well before"
        )
    ...
    def test_trigger_misfire_negation(self):
        ...
        is_misfire, reason = self.analyzer.detect_misfire("go", "I don't go anywhere without you")
        assert is_misfire is True
        assert "negated" in reason

    def test_trigger_valid(self):
        """Test valid trigger (no misfire)."""
        is_misfire, reason = self.analyzer.detect_misfire("stay", "Please stay with me")
...
class TestFallbackProtocol:
    ...
    def test_ambiguous_tone_exchange(self):
        ...
        assert (
            "stay" in result["companion_behavior"]["message"].lower()
            or "silent" in result["companion_behavior"]["message"].lower()
        )
