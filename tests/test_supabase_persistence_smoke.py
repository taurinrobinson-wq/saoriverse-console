...
def test_conversation_save_and_load(monkeypatch):
    ...
    mgr = ConversationManager(
        user_id, supabase_url='http://example.test', supabase_key='fake-key')

    fake_requests = FakeRequests()
    # Patch the requests module used inside the conversation_manager
    monkeypatch.setattr(
        'emotional_os.deploy.modules.conversation_manager.requests', fake_requests)

    # Create a fake conversation
    conv_id = str(uuid.uuid4())
    messages = [
        {'user': 'I feel anxious about the meeting', 'assistant': 'I hear you.'}
    ]
    title = 'Anxious about meeting'

    # Act: save conversation
    success, msg = mgr.save_conversation(
