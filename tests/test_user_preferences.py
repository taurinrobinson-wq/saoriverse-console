...
class FakeRequests:
    ...
    def get(self, url, headers=None, params=None, timeout=None):
        ...
        uid = user_id_param.split(
            'eq.')[-1] if 'eq.' in user_id_param else user_id_param
...
def test_save_and_load_user_preferences(monkeypatch):
    ...
    mgr = ConversationManager(
        user_id='test_user', supabase_url='https://example.supabase.co', supabase_key='testkey')

    success, msg = mgr.save_user_preferences(
        {'persist_history': True, 'persist_confirmed': True})
