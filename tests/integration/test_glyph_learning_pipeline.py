...
# Add paths
sys.path.insert(0, "str(Path(__file__).resolve().parent) OS")
...
def test_glyph_learning_pipeline():
    ...
    for i, test_case in enumerate(test_inputs, 1):
        ...
        candidate = learner.analyze_input_for_glyph_generation(
            input_text=test_case["text"], signals=signals, user_hash=user_hash
        )
        ...
        emotional_analysis = {
            ...
            "emotional_terms": {"intensity_words": ["caught"], "state_words": ["feel"], "relation_words": ["between"]},
        ...
        shared_mgr.record_glyph_adoption(
            user_hash=user_hash, glyph_name=candidate["glyph_name"], quality_rating=1  # Positive feedback (implicit)
        )
        ...
        user_glyphs = shared_mgr.get_glyphs_for_user(
            user_hash=user_hash, emotional_signal=candidate["emotional_signal"], gates=candidate["gates"]
        )
